...
export function toTitleCase(str) {
  ...
  str = str.replace(/([A-Za-z]\.[A-Za-z]\.)([A-Z][a-z]+)/g, (match, initials, word) => {
  ...
  return str.replace(/\S+/g, (word) => {
    ...
    if (word.includes('.')) {
      ...
      if (parts.length > 0 && parts.every(part => part.length <= 2 && /^[A-Za-z]*$/.test(part))) {
        ...
        const needsTrailingPeriod = hadTrailingPeriod && !reconstructed.endsWith('.')
    ...
    if (word.length <= 4 && !word.includes('.') && knownAcronyms.has(upperWord)) {
      return upperWord
    }
